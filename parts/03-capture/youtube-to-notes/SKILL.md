...
2. If it says yt-dlp is not installed, tell the person it is a free program that pulls the
   ...
   step 1 again. If installing fails, show the error and stop.

3. It prints one line per link. `saved` or `already here` gives the transcript's path.
   `no subtitles` means the video has none: say so and move on. `failed` gives the reason: say it
   plainly and move on. Never write a note for a video whose words you do not have.
...
4. Read the transcript. Then write the summary as one note in `Resources/`, named after the
   ...
   - **Transcript**: a link to the transcript note.
